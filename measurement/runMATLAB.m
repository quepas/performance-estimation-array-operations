% Change PAPI path !
addpath('/home/quepas/PhD/Tools/mPAPI');
...
fprintf('@1@ Measuring Array Operations...\n');
...
for k = 1:2:length(arrayOps)
    ...
    for aspect = config.ASPECTS
        ...
        runMATLABTest(config, arrayOps{k}, arrayOps{k+1}, aspect{1});
    end
end
...
for k = 1:2:length(examples)
    ...
    for aspect = config.ASPECTS
        fprintf('--> Measuring %s (aspect %s)\n', examples{k}, aspect{1});
        runMATLABTest(config, examples{k}, examples{k+1}, aspect{1});
    end
end
